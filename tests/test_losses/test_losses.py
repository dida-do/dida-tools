...
class TestPrecision(TestLoss):
    def test_worstcase(self):
        loss = precision(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert loss == 0.
    ...
    def test_order(self):
        small_loss = precision(self.seg_mask_logits, self.inverted_seg_mask_logits)
        medium_loss = precision(self.seg_mask_logits, self.half_inverted_logits)
...
class TestRecall(TestLoss):
    def test_worstcase(self):
        loss = recall(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert loss == 0.
    ...
    def test_order(self):
        small_loss = recall(self.seg_mask_logits, self.inverted_seg_mask_logits)
        medium_loss = recall(self.seg_mask_logits, self.half_inverted_logits)
        large_loss = recall(self.seg_mask_logits, self.seg_mask_logits)
        assert large_loss > medium_loss > small_loss

class TestF1Score(TestLoss):
    def test_worstcase(self):
        loss = f1(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert loss == 0.
    ...
    def test_order(self):
        small_loss = f1(self.seg_mask_logits, self.inverted_seg_mask_logits)
        medium_loss = f1(self.seg_mask_logits, self.half_inverted_logits)
        large_loss = f1(self.seg_mask_logits, self.seg_mask_logits)
        assert large_loss > medium_loss > small_loss

class TestSmoothDiceLoss(TestLoss):
    def test_worstcase(self):
        loss = smooth_dice_loss(self.seg_mask_logits, self.inverted_seg_mask_logits)
    ...
    def test_order(self):
        ...
        medium_loss = smooth_dice_loss(self.seg_mask_logits, self.half_inverted_logits)
        large_loss = smooth_dice_loss(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert large_loss > medium_loss > small_loss

class TestSmoothDiceBetaLoss(TestLoss):
    def test_worstcase(self):
        loss = smooth_dice_beta_loss(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert np.isclose(loss.item(), 1., atol=1e-2)
    ...
    def test_order(self):
        ...
        medium_loss = smooth_dice_beta_loss(self.seg_mask_logits, self.half_inverted_logits)
        large_loss = smooth_dice_beta_loss(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert large_loss > medium_loss > small_loss

class TestDiceBCESumLoss(TestLoss):
    def test_worstcase(self):
        loss = dice_bce_sum(self.seg_mask_logits, self.inverted_seg_mask_logits)
    ...
    def test_order(self):
        ...
        medium_loss = dice_bce_sum(self.seg_mask_logits, self.half_inverted_logits)
        large_loss = dice_bce_sum(self.seg_mask_logits, self.inverted_seg_mask_logits)
        assert large_loss > medium_loss > small_loss
